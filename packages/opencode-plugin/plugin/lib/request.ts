...
export type SahayakConfig = {
  instanceId: string
  baseUrl: string
}

export function getSahayakConfig(): SahayakConfig {
  return {
    instanceId: requireEnv("SAHAYAK_INSTANCE_ID"),
    baseUrl: requireEnv("SAHAYAK_BASE_URL"),
  }
}

export function createSahayakRequester(config: SahayakConfig) {
  const rawBaseUrl = (config.baseUrl ?? "").trim()
  const baseUrl = rawBaseUrl.replace(/\/+$/, "")
  const pluginBase = `${baseUrl}/api/opencode/${encodeURIComponent(config.instanceId)}/plugin`
  ...
  const buildHeaders = (headers: HeadersInit | undefined, hasBody: boolean): Record<string, string> => {
    ...
    if (hasBody) output["Content-Type"] = output["Content-Type"] ?? "application/json"
    return output
  }
  ...
  return { buildUrl, fetch: fetchWithAuth, requestJson, requestVoid, requestSseBody }
}
...
function requireEnv(key: string): string {
  ...
  if (!value || !value.trim()) throw new Error(`[SahayakPlugin] Missing required env var ${key}`)
  return value
}

function buildInstanceAuthorizationHeader(): string {
  const username = requireEnv("SAHAYAK_SERVER_USERNAME")
  const password = requireEnv("SAHAYAK_SERVER_PASSWORD")
