...
export function VoicePoweredOrb(props: VoicePoweredOrbProps) {
  ...
  onMount(() => {
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        program.uniforms.hue.value = props.hue ?? 0

        const voice = props.voiceLevel ?? 0

        if (voice > 0.05) {
          const targetRot = currentRot + dt * (0.3 + voice * 2.4)
          currentRot = targetRot
          currentHover = Math.min(voice * 2.0, 1.0)
        } else {
          currentHover = 0
        }

        program.uniforms.hover.value = currentHover
        program.uniforms.hoverIntensity.value = Math.min(voice * 0.8 * 0.8, 0.8)
